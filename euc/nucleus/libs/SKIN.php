...
/**
  ...
  *
  * $Id: SKIN.php,v 1.1.1.1 2005-02-28 07:14:14 kimitake Exp $
  */
class SKIN {
			...
			case 'blogsetting':
				if ($name == 'trackback') {
					...
					if ($plugin != NULL){
						...
						if ($value === '0') $value = 'no';
						if ($plugin->getOption('AcceptPing') == 'no' ) {
							$condition = ($value == 'no');
							} else {
							$tb_option = $plugin->getBlogOption($bid,'AllowTrackBack');
							if (!$tb_option) {
								$condition = ($value == 'yes');
							} else {
								$condition = ($tb_option == $value);
							}
						}
					}
					break;
