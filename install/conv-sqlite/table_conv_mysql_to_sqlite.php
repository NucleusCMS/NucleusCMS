...
//		$s = str_replace("\n," , ",\n", $s);
        ...
        if (
                (count($index_key) == 0) && (count($fulltext_key) == 0)
        )
            return $s;
...
//			$tablename = $m[1];
        ...
        foreach ($fulltext_key as $item)
        {
            ...
            $s .= "\n"
                    ...
                    . sprintf("CREATE VIRTUAL TABLE `%s` USING fts4(%s);\n\n", $vt_tablename
...
//		var_dump($s); exit;
    ...
    public function get_sql_create_trigger($table, $vt_table, $key_names)
    {
        if (preg_match('/nucleus_item$/ims', $table))
            $num = 'inumber';
        else if (preg_match('/nucleus_comment$/ims', $table))
            $num = 'cnumber';
        else
        ...
        $s .= 'CREATE TRIGGER ' . $table . '_trig_insert' . "\n"
                ...
                . "BEGIN\n  "
                . sprintf("INSERT INTO %s(docid, %s)", $vt_table
                        , 'ifts_' . implode(',ifts_', $key_names) . '')
                . sprintf(" VALUES(NEW.%s , %s);\n", $num
        ...
        $s .= 'CREATE TRIGGER ' . $table . '_trig_update' . "\n"
                ...
                . "BEGIN\n  "
                . sprintf("UPDATE %s SET %s WHERE docid = NEW.%s;\n", $vt_table
                        , implode(',', $sets)
                        , $num)
                . "END;\n\n";
        return $s;
