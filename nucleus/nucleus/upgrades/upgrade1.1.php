...
$query = "INSERT INTO nucleus_config VALUES ('CookiePath', '/');";
upgrade_query('CookiePath setting',$query);
$query = "INSERT INTO nucleus_config VALUES ('CookieDomain', '');";
upgrade_query('CookieDomain setting',$query);
$query = "INSERT INTO nucleus_config VALUES ('CookieSecure', '0');";
upgrade_query('CookieSecure setting',$query);
$query = "INSERT INTO nucleus_config VALUES ('MediaPrefix', '1');";
upgrade_query('MediaPrefix setting',$query);


// 2. add language field to member table
upgrade_query("Language setting (member)", 
		      "ALTER TABLE nucleus_member ADD deflang varchar(20) NOT NULL default '';");

// 3. create category table and update other tables (nucleus_item and nucleus_blog)
$query = "CREATE TABLE nucleus_category ("
...
$query = "ALTER TABLE nucleus_skin_desc ADD sdtype VARCHAR(40) NOT NULL DEFAULT 'text/html'";
upgrade_query("Adding content type field to skins (text/html)", $query);

// 6. try to set content type for xml-rss skin to text/xml
$query = "UPDATE nucleus_skin_desc SET sdtype='text/xml' WHERE sdname='xmlrss'";
...
while ($obj = mysql_fetch_object($res)) {
	...
	$query = "INSERT INTO nucleus_template VALUES ($tid, 'CATLIST_HEADER', '<ul><li><a href=\"<%blogurl%>\">All</a></li>');";
	$query2 = "INSERT INTO nucleus_template VALUES ($tid, 'CATLIST_LISTITEM', '<li><a href=\"<%catlink%>\"><%catname%></a></li>');";
	$query3 = "INSERT INTO nucleus_template VALUES ($tid, 'CATLIST_FOOTER', '</ul>');";
...
upgrade_query("Notify Type Setting", 
		      "ALTER TABLE nucleus_blog ADD bnotifytype INT(11) NOT NULL default '15';");
