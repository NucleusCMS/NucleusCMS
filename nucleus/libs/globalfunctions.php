...
if (PHP_VERSION_ID >= 90000) {
    exit('<h1>Error</h1><div>This version does not support PHP 9.0 or later.</div>
        <div>Nucleus CMS upgrade is required to work with PHP9.0 or later.</div>');
}
        ...
        for ($i = 0; $i < count($data); $i++) {
            switch ($data[$i]) {
                ...
                case $CONF['ArchiveKey']: // two possibilities: archive/yyyy-mm or archive/1/yyyy-mm (with blogid)
                    if ((($i + 1) < count($data)) && (!strstr($data[$i + 1], '-'))) {
