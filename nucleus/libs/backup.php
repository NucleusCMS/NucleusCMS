...
class Backup
{
    ...
    /*
     *   get current nucleus's all table name lists
     */
    public function getTablenameLists()
    {
        global $manager;
        ...
        foreach (['systemlog','member_option'] as $value) {
    ...
    public function do_backup($gzip = 0)
    {
        ...
        echo "# https://nucleuscms.github.io/ \n";
        echo "#\n";
        ...
        if ( ! $this->mode_en && defined('_BACKUP_BACKUPFILE_NUCLEUSVERSION')) {
            echo "# " . _BACKUP_BACKUPFILE_NUCLEUSVERSION . NUCLEUS_VERSION_DOT . "\n";
        } else {
            echo "# Nucleus CMS version: " . NUCLEUS_VERSION_DOT . "\n";
        }
        echo "#\n";
    ...
    public function convert_to_innodb()
    {
        ...
        foreach ($this->getTablenameLists() as $tablename) {
            $param['tablename'] = $tablename;
            if ($tablename === sql_table('systemlog')) {
                sql_prepare_execute("ALTER TABLE `{$tablename}` MODIFY `logid` BIGINT NOT NULL");
            }
            if (sql_direct_getValue_AsInt($sql, $param)) {
                // table Engine is not innodb
                sql_prepare_execute('ALTER TABLE '.sql_quote_identifier($tablename) . ' ENGINE=InnoDB;');
            }
        }
    }
}
