        ...
        case 'BODY':
            ...
            if (count($types) > 0) {
                ...
                for ($i = 0; $i < count($types); $i++) {
                    $type = $types[$i];
                    if (in_array($type,
                        array('index', 'item', 'archivelist', 'archive', 'search', 'error', 'member', 'imagepopup'))) {
                ...
                if (count($parts[1]) > 0) //                    echo _SKIN_PARTS_SPECIAL . ' <ul>'.implode($parts[1]).'</ul>';
                {
                    printf("<div style='display: inline-block; vertical-align: top; padding-left: 20px;'>%s</div>",
                        _SKIN_PARTS_SPECIAL . ' <ul>' . implode($parts[1]) . '</ul>');
                }
            }
            // skin page
