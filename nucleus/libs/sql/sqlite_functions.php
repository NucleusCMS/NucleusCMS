...
class sqlite_functions
{
    public static function pdo_register_user_functions($dbh) {
        ...
        $dbh->sqliteCreateFunction('SUBSTRING',
                function() { $st = intval(func_get_arg(1)); if ($st>0) $st--; return substr(func_get_arg(0) , $st , func_get_arg(2)); }
                , 3);
        $dbh->sqliteCreateFunction('UNIX_TIMESTAMP', 'strtotime', 1);
        $dbh->sqliteCreateFunction('NOW', function() { return date("Y-m-d H:i:s", time()); }, 0); // local time
        ...
        $dbh->sqliteCreateFunction('REGEXP',
                function($pattern , $Text) { return preg_match("/(".str_replace("/","\\/",(string) $pattern).")/im", (string) $Text) ? 1 : 0; }
                , 2);
        $dbh->sqliteCreateFunction('CONCAT', function() { return implode ("", func_get_args() ); }, -1);
        $dbh->sqliteCreateFunction('FIND_IN_SET', function($k,$v) { return in_array($k, explode($v)) ? 1 : 0; }, 2);
