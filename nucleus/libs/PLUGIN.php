...
class NucleusPlugin {
    ...
    function _getOption($context, $contextid, $name) {
        ...
        $ph['oid'] = (int)$oid;
        $ph['ocontextid'] = (int)$contextid;
        $has_ovalue = sql_query(parseQuery('SELECT ovalue FROM [@prefix@]plugin_option WHERE oid=[@oid@] AND ocontextid=[@ocontextid@]',
            $ph));
        $has_row = (int)parseQuickQuery('SELECT count(*) FROM [@prefix@]plugin_option WHERE oid=[@oid@] AND ocontextid=[@ocontextid@] LIMIT 1',
            $ph);
    ...
    function _getOID($context, $name) {
        ...
        $res = sql_query(parseQuery('SELECT oid, oname, ocontext, odef FROM [@prefix@]plugin_option_desc WHERE opid=[@opid@]',
            $ph));
        if ($res) {
            while ($o = sql_fetch_object($res)) {
    ...
    public static function _deleteOptionValues($context, $contextid) {
        ...
        if (count($aOIDs) > 0) {
            $ph = array('aOIDs' => join(',', $aOIDs), 'ocontextid' => (int)$contextid);
    ...
    public static function _applyPluginOptions(&$aOptions, $newContextid = 0) {
        ...
        foreach ($aOptions as $oid => $values) {
            ...
            $ph = array('oid' => (int)$oid);
            $res = sql_query(parseQuery('SELECT opid, oname, ocontext, otype, oextra, odef FROM [@prefix@]plugin_option_desc WHERE oid=[@oid@]',
                $ph));
